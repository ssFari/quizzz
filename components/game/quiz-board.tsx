...
import { Check, Clock, Heart, MessageCircle, Send, X } from "lucide-react";
import { AnimatePresence, motion } from "motion/react";
import { useState } from "react";
import { TutorPanel } from "@/components/game/tutor-panel";
import { Sheet, SheetContent, SheetTrigger } from "@/components/ui/sheet";
import type { UseQuiz } from "@/hooks/game/use-quiz";
...
/**
 * Presentational quiz board shared by the built-in worlds and teacher-authored
 * sets. All gameplay state lives in `quiz`; this only renders it.
 */
	...
	return (
		<div className="relative mx-auto flex min-h-dvh max-w-md flex-col overflow-hidden bg-slate-50">
			...
			<div className="flex-1 overflow-y-auto p-5 pb-28">
				...
				{q.type === "short" ? (
					<ShortAnswer
						...
						theme={theme}
						disabled={answered}
						onSubmit={(value) => quiz.handleAnswer(value)}
					/>
				) : (
			...
			<Sheet>
				...
				>
					<TutorPanel tutor={tutor} questionContext={q.prompt} />
				</SheetContent>
			</Sheet>
					...
					>
						...
						<div className="mb-5 rounded-2xl border border-slate-100 bg-white/80 p-4">
							...
							<p className="font-semibold leading-relaxed text-slate-700">
								{q.explanation || "Cocokkan dengan jawaban benar di atas ya."}
							</p>
						</div>
						<button
							type="button"
	...
	return (
		...
		>
			<input
				...
				className="w-full rounded-2xl border-2 border-slate-200 bg-white px-4 py-4 text-lg font-bold text-slate-700 outline-none transition focus:border-indigo-400 disabled:opacity-60"
			/>
			<button
				type="submit"
				disabled={disabled || value.trim().length === 0}
				className={cn(
					"flex items-center justify-center gap-2 rounded-2xl py-4 font-heading text-lg font-extrabold text-white transition active:translate-y-1 active:shadow-none disabled:opacity-50",
					theme.solid,
					theme.shadow,
				)}
			>
				<Send className="size-5" /> Jawab
			</button>
		</form>
	);
}
